...
function [optimal_rho,optimal_multiplier,solution] = ROAMultiplierProg(dV,V,inequalities,...
    equalities,rho_bounds,method,deg,options)
...
progMethod = @SprocedureEqualitiesProg; evalMethod = @evalROAProgDSOS;
...
while ~(rho_try-solution.rho <= 0.01 && rho_try ~= rho_initial) && ...
    ...
    [sol,decision_multiplier] = progMethod(-dV,[(rho_try-V),inequalities],equalities,deg,options);
