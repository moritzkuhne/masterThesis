...
function [solution,Qset] = SprocedureProg(poly,inequalities,equalities,deg,options)
...
%   Detailed explanation goes here
    ...
    %setting up free poly multiplier
    [prog,h,coeff] = prog.newFreePoly(monomials(indet,0:2*deg),length(equalities));
    ...
    for i=1:length(equalities)
        ...
        if ~exist('E', 'var')
            ...
            E = E + h(i)*equalities(i);
        end
        
    end 

    % DSOS constraint
    prog = prog.withDSOS((poly-S-E));
